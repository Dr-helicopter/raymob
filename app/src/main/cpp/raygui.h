...
*                         REVIEWED: GuiScrollPanel(), terminal_scroll speed proportional to content
...
*                         REVIEWED: GuiScrollPanel() to avoid smallish terminal_scroll-bars
...
typedef enum {
    ...
    SCROLL_PADDING,             // ScrollBar terminal_scroll padding from arrows
...
int GuiScrollPanel(Rectangle bounds, const char *text, Rectangle content, Vector2 *scroll, Rectangle *view)
{
    ...
    // Make sure terminal_scroll bars have a minimum width/height
    ...
    *view = (GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)?
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        if (CheckCollisionPointRec(mousePoint, bounds))
        {
            ...
            else scrollPos.y += wheelMove*mouseWheelSpeed.y; // Vertical terminal_scroll
        }
    }

    // Normalize terminal_scroll values
    ...
    // Draw detail corner rectangle if both terminal_scroll bars are visible
...
int GuiListViewEx(Rectangle bounds, const char **text, int count, int *scrollIndex, int *active, int *focus)
{
    ...
    // Check if we need a terminal_scroll bar
    ...
    if (useScrollBar)
    {
        ...
        int prevScrollSpeed = GuiGetStyle(SCROLLBAR, SCROLL_SPEED); // Save default terminal_scroll speed
        GuiSetStyle(SCROLLBAR, SCROLL_SLIDER_SIZE, (int)sliderSize);            // Change slider size
        GuiSetStyle(SCROLLBAR, SCROLL_SPEED, count - visibleItems); // Change terminal_scroll speed

        startIndex = GuiScrollBar(scrollBarBounds, startIndex, 0, count - visibleItems);

        GuiSetStyle(SCROLLBAR, SCROLL_SPEED, prevScrollSpeed); // Reset terminal_scroll speed to default
